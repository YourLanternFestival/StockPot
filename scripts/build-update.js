...
const batContent = [
  ...
  'chcp 65001 >nul',
  `title 食堂出入库管理系统 - 增量更新 v${newVersion}`,
  'echo.',
  'echo ========================================',
  'echo   食堂出入库管理系统 增量更新',
  ...
  ':: 查找安装目录',
  'set "INSTALL_DIR="',
  'if exist "%LOCALAPPDATA%\\Programs\\食堂出入库管理系统" (',
  '  set "INSTALL_DIR=%LOCALAPPDATA%\\Programs\\食堂出入库管理系统"',
  ...
  '  if not exist "!INSTALL_DIR!\\食堂出入库管理系统.exe" (',
  '    echo [错误] 该目录下未找到食堂出入库管理系统.exe',
  '    pause',
  '    exit /b 1',
  '  )',
  ')',
  ...
  '',
  ':: 关闭正在运行的应用',
  'taskkill /f /im "食堂出入库管理系统.exe" >nul 2>&1',
  'if %errorlevel% equ 0 (',
  '  echo [OK] 已关闭正在运行的应用',
  ') else (',
  '  echo [OK] 应用未在运行',
  ...
  'echo 按任意键启动应用...',
  'pause >nul',
  'start "" "%INSTALL_DIR%\\食堂出入库管理系统.exe"',
].join('\r\n');
